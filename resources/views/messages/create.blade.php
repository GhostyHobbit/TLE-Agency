<x-nav>
    ...
    <div class="max-w-4xl mx-auto p-8 bg-white shadow-xl rounded-xl mt-10 mb-16">
        ...
        @if (session('success'))
            <p class="text-yellow">{{ session('success') }}</p>
        @endif
        ...
        <form id="messageForm" action="{{ route('messages.store', $vacancy->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="number_of_invites" id="number_of_invites" min="1" class="mt-2 block w-full bg-mossLight border-mossMedium rounded-md shadow-sm px-4 py-3 focus:ring-mossMedium focus:border-mossMedium" required>
            </div>

            <!-- Bericht -->
            <div class="mb-6">
                <label for="message" class="block text-sm font-medium text-black">Schrijf uw bericht:</label>
                <textarea name="message" id="message" rows="5" class="mt-2 block w-full bg-mossLight border-mossMedium rounded-md shadow-sm px-4 py-3 focus:ring-mossMedium focus:border-mossMedium" required></textarea>
            </div>

            <!-- Locatie -->
            <div class="mb-6">
                <label for="location" class="block text-sm font-medium text-black">Op welke locatie wilt u afspreken?</label>
                <input type="text" name="location" id="location" class="mt-2 block w-full bg-mossLight border-mossMedium rounded-md shadow-sm px-4 py-3 focus:ring-mossMedium focus:border-mossMedium" required>
            </div>
            ...
            <div class="mb-6 grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="date" id="date" class="mt-2 block w-full bg-mossLight border-mossMedium rounded-md shadow-sm px-4 py-3 focus:ring-mossMedium focus:border-mossMedium" required>
                </div>
                <div>
                    <label for="time" class="block text-sm font-medium text-black">Tijd:</label>
                    <input type="time" name="time" id="time" class="mt-2 block w-full bg-mossLight border-mossMedium rounded-md shadow-sm px-4 py-3 focus:ring-mossMedium focus:border-mossMedium" required>
                </div>
            </div>
